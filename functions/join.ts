...
const expectedAction = 'join-discord';
...
export async function onRequest({ env, request }: Context): Promise<Response> {
	if (request.method === 'POST') {
		...
		try {
			...
			if (valid) {
				console.log('turnstile verified', result.hostname);
    ...
    <script>
      (function () {
        ...
        window.onTurnstileError = function (code) {
          var el = document.getElementById('turnstile-error');
          if (el) {
            el.hidden = false;
            el.textContent = 'Verification could not load (code ' + code + '). Please retry.';
          }
        };
      })();
    </script>
  ...
  <body>
    ...
    <script>
      (function () {
        ...
        if (el) el.setAttribute('data-theme', document.documentElement.classList.contains('dark') ? 'dark' : 'light');
      })();
    </script>
